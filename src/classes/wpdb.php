...
class wpdb {
    ...
    protected function get_table_from_query( $query ) {
        ...
        $query = preg_replace( '/\((?!\s*select)[^(]*?\)/i', '()', $query );
		 ...
		 */
        if ( preg_match( '/^\s*SHOW\s+(?:TABLE\s+STATUS|(?:FULL\s+)?TABLES)\s+(?:WHERE\s+Name\s+)?LIKE\s*("|\')((?:[\\\\0-9a-zA-Z$_.-]|[\xC2-\xDF][\x80-\xBF])+)%?\\1/i', $query, $maybe ) ) {
     ...
     * @return array|null Column results.
     */
